...
use crate::{Error, HandshakeData, KeyLog, KeyLogLabel, Level, QuicSsl, QuicVersion, retry, SslError};
...
impl SessionState {
    ...
    pub(crate) fn handshake_data(&self) -> Option<Box<dyn Any>> {
        let sni_name = if self.side.is_server() {
            match self.ssl.servername(NameType::HOST_NAME) {
        ...
        let alpn_protocol = match self.ssl.selected_alpn_protocol() {
    ...
    pub(crate) fn early_crypto(
        ...
        let header_key = early_secret
            ...
            .as_crypto()
            .unwrap();
        let packet_key = Box::new(early_secret
