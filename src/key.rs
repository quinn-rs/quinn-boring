...
impl KeyPair<PacketKey> {
    ...
    pub(crate) fn as_crypto(&self) -> Result<crypto::KeyPair<Box<dyn crypto::PacketKey>>> {
        Ok(crypto::KeyPair {
            local: Box::new(self.local.clone()),
            remote: Box::new(self.remote.clone()),
        })
    }
}
...
impl crypto::PacketKey for PacketKey {
    ...
    fn encrypt(&self, packet_number: u64, buf: &mut [u8], header_len: usize) {
        ...
        self.aead_key.seal_in_place(&nonce, &mut payload_tag, &header)
            .unwrap();
    }
    ...
    fn decrypt(
        ...
        let plain_len = self.aead_key.open_in_place(&nonce, payload.as_mut(), header)?;
