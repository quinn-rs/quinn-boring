...
use crate::{
    bffi_ext, param_util, Entry, KeyLog, NoKeyLog, QuicSsl, QuicSslSession, SessionCache,
    SimpleCache,
};
use boring::pkey::{PKey, Private};
use boring::ssl::{
    Ssl, SslContext, SslContextBuilder, SslMethod, SslSession, SslVerifyMode, SslVersion,
};
...
impl Config {
    ...
    /// Utility method that sets the certificate chain and private key for this client.
    pub fn set_cert(&mut self, cert_chain: Vec<X509>, key: PKey<Private>) -> Result<()> {
        bffi_ext::set_ctx_cert(&mut self.ctx, cert_chain, key)
    }

    /// Adds the given certificate to the trusted cert store. The server will be validated
    /// against the trusted certs.
    ...
    pub fn set_alpn(&mut self, alpn_protocols: &[Vec<u8>]) -> Result<()> {
        self.ctx.set_alpn_protos(&AlpnProtocols::from(alpn_protocols).encode())?;
        Ok(())
    }
