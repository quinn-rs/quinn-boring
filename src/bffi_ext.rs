...
use boring::pkey::{HasPrivate, PKey, Private};
...
pub trait QuicSslContext {
    ...
    fn add_extra_chain_cert(&mut self, cert: X509) -> BoringResult;
...
/// Utility method that sets the certificate chain and private key on the context.
pub(crate) fn set_ctx_cert(
    ctx: &mut SslContext,
    cert_chain: Vec<X509>,
    key: PKey<Private>,
) -> crate::Result<()> {
    if cert_chain.is_empty() {
        return Err(Error::invalid_input("certificate chain is empty".into()));
    }

    // Store the first in the chain as the leaf certificate.
    ctx.set_certificate(cert_chain[0].clone())?;

    // Store the rest of the certs in the chain.
    for cert in &cert_chain[1..] {
        ctx.add_extra_chain_cert(cert.clone())?;
    }
