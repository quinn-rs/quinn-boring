...
use crate::{bffi_ext, param_util, retry, KeyLog, NoKeyLog, QuicSslContext};
...
impl Config {
    ...
    /// Utility method that sets the certificate chain and private key for this server.
    pub fn set_cert(&mut self, cert_chain: Vec<X509>, key: PKey<Private>) -> Result<()> {
        bffi_ext::set_ctx_cert(&mut self.ctx, cert_chain, key)
    }

    /// Adds the given certificate to the trusted cert store. If [use_client_auth] was
    /// enabled, the client will be validated against the trusted certs.
    pub fn add_trusted_cert(&mut self, cert: X509) -> Result<()> {
        self.ctx.cert_store_mut().add_cert(cert)?;
        Ok(())
    }

    /// Sets whether or not the server will perform validation of the client. If enabled,
    /// the server will validate the client using the trusted roots set by [add_trusted_cert].
    pub fn enable_client_auth(&mut self, enable: bool) {
        self.client_auth = enable;
    }
...
impl Session {
    fn new(
        ...
        ssl.set_quic_early_data_context(&param_util::encode_early_data_context(params, &None))
            .unwrap();
